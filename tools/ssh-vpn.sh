...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/ndmservis/permission/main/ip > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/ndmservis/permission/main/ip | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/ndmservis/permission/main/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
wget -q -O /usr/local/bin/ws-stunnel "https://raw.githubusercontent.com/ndmservis/autoscript/main/tools/ws-stunnel"
...
# Install Stunnel5
cd /root/
wget -q "https://raw.githubusercontent.com/ndmservis/autoscript/main/tools/stunnel5.zip"
unzip stunnel5.zip
cd /root/stunnel
chmod +x configure
./configure
...
wget -q -O /etc/init.d/stunnel5 "https://raw.githubusercontent.com/ndmservis/autoscript/main/tools/stunnel5.init"
...
wget -q -O /etc/issue.net "https://raw.githubusercontent.com/ndmservis/autoscript/main/issue.net"
